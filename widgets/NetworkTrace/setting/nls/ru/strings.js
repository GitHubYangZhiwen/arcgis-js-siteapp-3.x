define({
  ...
  "projectSetting": {
    ...
    "note": "Примечание: Параметры проекта являются необязательными, после настройки пользователь может сохранить проект в нужных слоях веб-карты с областью отключения сети и входными параметрами. Пользователь может сохранить другие выходные параметры из вкладки «Выходные».",
  ...
  "summaryTab": {
    ...
    "saveToLayerHelp": "<p>Будут показаны слои со следующими условиями:<br/><ul><li>У слоя должны быть возможности редактирования “Создание”, “Удаление” и “Обновление”</li><li>У слоя должно быть два поля с точными именами и типами данных:</li><ul><li>parametername (Строковый тип поля)</li><li>projectid (поле типа Guid)</li></ul></ul><p/>",
